...
export function Note({ children, variant = 'info', title, className, icon, onClose }: NoteProps) {
    const config = variantConfig[variant];
    const IconComponent = icon || config.icon;
    const displayTitle = title || config.title;

    return (
        <div className={cn('relative rounded-lg border p-4', config.bg, config.border, className)} role="alert">
            <div className="flex gap-3">
                {IconComponent && (
                    <div className={cn('flex h-6 w-6 shrink-0 items-center justify-center rounded-full', config.iconBg)}>
                        <IconComponent className={cn('h-4 w-4', config.text)} />
                    </div>
                )}

                <div className="flex-1 space-y-1">
                    {displayTitle && <h4 className={cn('font-semibold', config.text)}>{displayTitle}</h4>}
